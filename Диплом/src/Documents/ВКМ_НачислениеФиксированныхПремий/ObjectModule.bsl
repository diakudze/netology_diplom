#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Процедура СоздатьДвиженияУдержаний(СтавкаНДФЛ);
		...
		Если ЗначениеЗаполнено(ТЗ_Список) Тогда  
			...
			Для Каждого Строка Из ТЗ_Список Цикл
				...
				Движение_Удержания.ВидРасчетаНачисление = ПланыВидовРасчета.ВКМ_ДополнительныеНачисления.Премия;
	...
	Процедура РассчитатьНДФЛ()
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |ПОМЕСТИТЬ ВТ_База
		               |ИЗ
		               |	РегистрРасчета.ВКМ_Удержания.БазаВКМ_ДополнительныеНачисления(&Измерения, &Измерения, &Разрезы, Регистратор = &Ссылка) КАК ВКМ_УдержанияБазаВКМ_ДополнительныеНачисления
		               |
		               |СГРУППИРОВАТЬ ПО
		               |	ВКМ_УдержанияБазаВКМ_ДополнительныеНачисления.СтавкаНДФЛ,
		               |	ВКМ_УдержанияБазаВКМ_ДополнительныеНачисления.ВидРасчетаРазрез,
