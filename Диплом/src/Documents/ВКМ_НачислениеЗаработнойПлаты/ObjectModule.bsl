#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Процедура СоздатьДвиженияУдержаний(СтавкаНДФЛ);
		...
		ТЗ_данные.Свернуть("Сотрудник, ВидРасчета",);
		...
		Если ЗначениеЗаполнено(ТЗ_данные) Тогда  
			...
			Для Каждого Строка Из ТЗ_данные Цикл
				...
				Движение_Удержания.ВидРасчетаНачисление = Строка.ВидРасчета;
				...
				//Если Строка.ВидРасчета = ПланыВидовРасчета.ВКМ_ОсновныеНачисления.Отпуск Тогда
				//	Движение_Удержания.БазовыйПериодНачало =  НачалоМесяца(ДобавитьМесяц(Дата,-12));
				//	Движение_Удержания.БазовыйПериодКонец = КонецМесяца(ДобавитьМесяц(Дата,-1)); 
				//Иначе
				//	Движение_Удержания.БазовыйПериодНачало = НачалоМесяца(Дата);
				//	Движение_Удержания.БазовыйПериодКонец  = КонецМесяца(Дата)-1;  
				//КонецЕсли;  
	...
	Процедура РассчитатьНДФЛ()
		...
		|	ВКМ_УдержанияБазаВКМ_ОсновныеНачисления.ВидРасчетаРазрез,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВКМ_Удержания.ВидРасчетаНачисление КАК ВидРасчетаНачисление,
		|	ВКМ_Удержания.Сотрудник КАК Сотрудник,
		|	ВКМ_Удержания.НомерСтроки КАК НомерСтроки
		|ПОМЕСТИТЬ ВТ_ФизТабУдержания
		|ИЗ
		...
		|
		|СГРУППИРОВАТЬ ПО
		|	ВКМ_Удержания.ВидРасчетаНачисление,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_ФизТабУдержания.ВидРасчетаНачисление КАК ВидРасчетаНачисление,
		|	ВТ_ФизТабУдержания.Сотрудник КАК Сотрудник,
		|	ВТ_ФизТабУдержания.НомерСтроки КАК НомерСтроки,
		|	ВТ_База.База КАК База,
		|	ВТ_База.СтавкаНДФЛ КАК СтавкаНДФЛ
		|ИЗ
		|	ВТ_ФизТабУдержания КАК ВТ_ФизТабУдержания
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_База КАК ВТ_База
		|		ПО ВТ_ФизТабУдержания.Сотрудник = ВТ_База.Сотрудник
		|			И ВТ_ФизТабУдержания.ВидРасчетаНачисление = ВТ_База.ВидРасчетаРазрез";
		...
		//Запрос.УстановитьПараметр("ФиксПремия", ПланыВидовРасчета.ВКМ_ДополнительныеНачисления.Премия);
	...
	Функция РасчетСуммыОкладаИСуммыПроцентОтВыплаты()
		...
		|	ВТ_Предварительно.Факт КАК Факт,
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ВКМ_ВыполненныеСотрудникомРаботы.Обороты(НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ), ДОБАВИТЬКДАТЕ(КОНЕЦПЕРИОДА(&Период, МЕСЯЦ), МИНУТА, 1), , ) КАК ВКМ_ВыполненныеСотрудникомРаботыОбороты
	...
	Процедура РасчетОтпуска();
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	МИНИМУМ(ДополнительныеНачисления.Сумма) КАК Сумма,
